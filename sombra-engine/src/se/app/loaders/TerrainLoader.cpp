...
namespace se::app {
	...
	Entity TerrainLoader::createTerrain(
		...
		const std::vector<float>& lodDistances, const SplatmapMaterial& terrainMaterial, const char* programName
	) {
...
// Private functions
	graphics::RenderableTerrain TerrainLoader::createTerrainRenderable(
		...
		auto gBufferRenderer = static_cast<graphics::Renderer*>(mGraphicsEngine.getRenderGraph().getNode("gBufferRenderer"));
		auto terrainPass = mCameraSystem.createPass3D(gBufferRenderer, program);
