...
namespace se::app {
	 ...
	 * perspective. It also has a "depthTexture" input where a depth Texture
	 * must be attached. This texture is the one rendererd from the camera
	 * perspective and it's used for calculating the final shadow map.
	 */
	...
	private:	// Attributes
		...
		/** The bindable index of the depth buffer texture used for rendering
		 * the Shadows */
		std::size_t mDepthTextureBindableIndex;
	...
	public:		// Functions
		...
		/** TODO: */
		std::size_t addShadow(std::size_t resolution, const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix);

		/** TODO: */
		void setShadowVPMatrix(std::size_t shadowIndex, const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix);
