	...
	void PlayerController::update(float deltaTime)
	{
		...
		if (mClicked) {
			...
			for (const auto& [entity, rayCast] : mGameData.collisionSystem->getEntities(transforms->position, forward)) {
				...
				mGameData.entityDatabase->addComponent(entity, std::move(transforms));

				se::graphics::RenderableMesh r3d(mTetrahedronMesh);
				r3d.addTechnique(mYellowTechnique);
				mGameData.entityDatabase->addComponent(pointEntity, std::move(r3d));
